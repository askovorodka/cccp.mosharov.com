...
if (mail("andrey.schmitz@gmail.com","test","message",$headers))
	echo 1;
exit();*/
...
if (isset($_POST['submit_import']))
{
	...
	if ($_FILES['import_file']['name']!='') {
		...
		if ($import->convert($xlsfile, $csvfile))
		{
			...
			{
				$db->query("update fw_products set disk_sklad = '0'");
...
/****************************************************************/
	...
	CASE ($action=='orders'):
		...
		$orders_list=$db->get_all("
			SELECT id,user,status,status as status_number,insert_date,total_price, dostavka,
			(SELECT name FROM fw_users WHERE id=o.user) AS user_name, 
			(SELECT SUM(b.product_price * b.product_count) 
		...
		for ($i=0;$i<count($orders_list);$i++) {
			...
			if ($orders_list[$i]['dostavka'] != 2)
			{
				if ($orders_list[$i]['total_products_price'] < SHOP_DOSTAVKA_LIMIT)
					$orders_list[$i]['total_products_price'] += SHOP_DOSTAVKA_PRICE;
			}
		}
	...
	CASE ($action=='order_details'):
		...
		$user_info=$db->get_single("SELECT a.*,b.comments,b.status, b.dostavka, b.payment, b.inn, b.kpp, b.company, b.company_dost 
									FROM fw_users as a INNER JOIN fw_orders as b
		...
		if ($user_info['company_dost'] > 0)
			$user_info['company_dost_name'] = $companies[$user_info['company_dost']];
		
		$smarty->assign("user_info",$user_info);

		$orders=$db->get_all("SELECT a.*,
		b.product_id as product_id, b.product_price,
		c.parent as parent,
		c.hit as hit,
		b.product_count,
		c.price as price,
		(b.product_price * product_count) as total_summ,
		order_id,
		product_count,
		c.name 	
		FROM `fw_orders` as a 
		INNER JOIN	(fw_orders_products as b INNER JOIN fw_products as c ON b.product_id=c.id)	ON a.id=b.order_id 
		WHERE a.id='$id'");
		...
		if ($user_info['dostavka'] != 2)
			if ($order_price < SHOP_DOSTAVKA_LIMIT)
